...
export default class Newspagecomp extends Component<AppProps,AppState> {
  ...
  componentDidMount(){
    fetch(apiurl+'/backend/'+this.props.url_slug+'/getnews/'+String(this.state.currentPage)+'/',
      {method: 'GET'}
    ).then(
  ...
  leftClick = () => {
    fetch(apiurl+'/backend/'+this.props.url_slug+'/getnews/'+String(this.state.currentPage-1)+'/',
      {method: 'GET'}
    ).then(
      response => response.json()
      ).then(result =>{
